...
def predict_vm (ecs_lines , input_lines) :
    ...
    dict_cpu = {"flavor1": 1, "flavor2": 1, "flavor3": 1, "flavor4": 2, "flavor5": 2, "flavor6": 2, "flavor7": 4,
                    "flavor8": 4, "flavor9": 4, "flavor10": 8, "flavor11": 8, "flavor12": 8, "flavor13": 16,
                    "flavor14": 16, "flavor15": 16};
    dict_mem = {"flavor1": 1024, "flavor2": 2048, "flavor3": 4096, "flavor4": 2048, "flavor5": 4096,
                    "flavor6": 8192, "flavor7": 4096, "flavor8": 8192, "flavor9": 16384, "flavor10": 8192,
                    "flavor11": 16384, "flavor12": 32768, "flavor13": 16384, "flavor14": 32768, "flavor15": 65536}
        # 按cpu分组
    cpu_16 = []
    cpu_8 = []
    cpu_4 = []
    ...
    while i <= fact_vm_number:
        ...
        while k < len(record):
            if int(record[k]) == i:  # 判断是否部署在第i+1台服务器上
                temporary[int(record[k + 1].replace("flavor", ""))] = temporary[int(record[k + 1].replace("flavor", ""))] + 1  # 用下标记录对应flavor ,下表的内容对应放置个数
            k = k + 2
...
def exp_sm(data,t_start,t_end):
    ...
    for i in range(len(data)):
        error.append(data[i][2]-s2[i])
    a=2*s1[-1]-s2[-1]
    b=alpha*(s1[-1]-s2[-1])/(1-alpha)
    return int(round((a+t_end*b)-(a+t_start*b)+13))
def nc(data):
    num=[]
    for line in data:
        num.append(line[2])
