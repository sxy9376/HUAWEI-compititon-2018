...
def exp_sm(data,t_start,t_end):
    ...
    b=alpha*((6-5*alpha)*s1[-1]-(10-8*alpha)*s2[-1]+(4-3*alpha)*s3[-1])/(2*((1-alpha)**2))
    c=((alpha/(1-alpha))**2)*(s1[-1]-2*s2[-1]+s3[-1])/2
    for i in range(len(data)):
        print (s3[i]-data[i][2])
    print(" ")
    return int(round((a+t_end*b+c*(t_end**2))-(a+t_start*b+c*(t_start**2))))
def nc(data):
    num=[]
    for line in data:
        num.append(line[2])
